...
class VectorDBTSNEVisualization(AbstractVisualization):
    ...
    def __init__(
        ...
        *,
        ...
        self.metadata_unique_key = metadata_unique_key
        self.is_3d = is_3d
    ...
    def draw(self):
        if self.collection:
            ...
            if self.metadata_unique_key:
                unique_types = list(
                    {
                        metadata[self.metadata_unique_key]
                ...
                colors = [
                    type_colors[metadata[self.metadata_unique_key]]
                    for metadata in collection_data["metadatas"]
                ] + (["red"] if self._question and self.show_question else [])
            else:
                colors = (["red"] * len(collection_data["metadatas"])) + (
                    ["red"] if self._question and self.show_question else []
                )
